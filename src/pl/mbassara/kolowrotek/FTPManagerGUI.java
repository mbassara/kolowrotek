...
public class FTPManagerGUI extends JFrame implements ActionListener, DocumentListener, PropertyChangeListener{
	...
	public FTPManagerGUI(){
		...
		chooseImagesAskLabel = new JLabel("Wybierz zdjęcia które chcesz dodać   ->");
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getSource().equals(exitButton)){
		...
		else if(e.getSource().equals(genModeButton) && genModeButton.isSelected()){
		...
		else if(e.getSource().equals(delModeButton) && delModeButton.isSelected()){
			...
			if(chooser.showOpenDialog(this) == JFileChooser.APPROVE_OPTION
				...
				chooseImagesAskLabel.setText("Wybrano " + filesList.length + " plików.");
				currentDir = filesList[0].getParentFile();
			}
			
			log.log(Level.INFO, chooser.getSelectedFiles().length + " files selected");
			...
			if(filesList == null && genModeButton.isSelected()){
				JOptionPane.showMessageDialog(this, "Wybierz zdjęcia!", "Nie tak szybko...", JOptionPane.WARNING_MESSAGE);
				log.log(Level.WARNING, "images were not selected - error");
			}
			else if(!partyTextChanged){
				JOptionPane.showMessageDialog(this, "Wpisz nazwę imprezy!", "Nie tak szybko...", JOptionPane.WARNING_MESSAGE);
				log.log(Level.WARNING, "party name was not entered - error");
			}
			else if(passwordText.getPassword().length == 0){
				JOptionPane.showMessageDialog(this, "Wpisz hasło!", "Nie tak szybko...", JOptionPane.WARNING_MESSAGE);
				log.log(Level.WARNING, "password was not entered - error");
			}
			else if(!genModeButton.isSelected() && !delModeButton.isSelected()){
				JOptionPane.showMessageDialog(this, "Wybierz tryb pracy!", "Nie tak szybko...", JOptionPane.WARNING_MESSAGE);
				...
				task = new FTPManager(mode, new String(passwordText.getPassword()),
				...
				log.log(Level.INFO, (genModeButton.isSelected() ? "generating" : "removing") + " mode is ready to start");
				task.execute();
				log.log(Level.INFO, (genModeButton.isSelected() ? "generating" : "removing") + " mode was executed");
			}
		}
	}
	...
	public void propertyChange(PropertyChangeEvent evt) {
		if(evt.getPropertyName().equals("error")){
			if(evt.getNewValue().equals("partyNotFound")){
				JOptionPane.showMessageDialog(this, "Nie istnieje impreza o takiej nazwie.",
													"Błąd!", JOptionPane.ERROR_MESSAGE);
				log.log(Level.WARNING, "party for entered name does not exists");
			}
			else if(evt.getNewValue().equals("dirExists")){
				JOptionPane.showMessageDialog(this, "Istnieje na serwerze folder o nazwie: " + FTPManager.createDirName(partyNameText.getText()) + "\n" +
													...
													"Błąd!", JOptionPane.ERROR_MESSAGE);
				log.log(Level.WARNING, "dir for entered party name already exists");
			}
			else{
				JOptionPane.showMessageDialog(this, "Błąd podczas przesyłania plików. Sprawdź podane hasło.\n" +
